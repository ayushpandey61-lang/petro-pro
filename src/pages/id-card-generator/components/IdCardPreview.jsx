...
const IdCardPreview = React.forwardRef(({ side, content, images, styles, toolElements = [] }, ref) => {
    const cardStyle = {
        ...
        return toolElements
            ...
            .map((element) => {
                ...
                switch (element.type) {
                        ...
                        return (
                            ...
                            >
                                <svg width="100%" height="100%" viewBox="0 0 100 20">
                                    <path
                                        d={
                                            element.pattern === 'sine'
                                                ? "M0,10 Q25,0 50,10 T100,10"
                                                : element.pattern === 'square'
                                                ? "M0,10 L25,10 L25,0 L75,0 L75,10 L100,10"
                                                : element.pattern === 'triangle'
                                                ? "M0,10 L50,0 L100,10"
                                                : element.pattern === 'sawtooth'
                                                ? "M0,10 L50,0 L50,10 L100,0"
                                                : "M0,10 L25,5 L50,15 L75,5 L100,10" // zigzag
                                        }
                    ...
                    case 'shape':
                        const ShapeIcon = element.shape === 'circle' ? '●' :
                    ...
                    case 'effect':
                        const effectStyle = {
                            ...
                            height: '50px',
                            borderRadius: '50%',
                            background: element.effect === 'glow'
                                ? `radial-gradient(circle, ${element.color}40 0%, transparent 70%)`
                                : element.effect === 'shadow'
                                ? `0 0 ${element.intensity}px ${element.color}`
                                : 'none',
                            filter: element.effect === 'blur' ? `blur(${element.intensity / 10}px)` : 'none',
                        };

                        if (element.effect === 'sparkle') {
                            return (
                                <div key={element.id} style={effectStyle}>
                                    ✨
                                </div>
                            );
                        } else if (element.effect === 'lightning') {
                            return (
                                <div key={element.id} style={effectStyle}>
                                    ⚡
                                </div>
