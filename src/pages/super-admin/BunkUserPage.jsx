...
const BunkUserPage = () => {
    ...
    const fetchRoles = useCallback(async () => {
        try {
            ...
            if (data.error) {
                toast({ title: "Error fetching roles", description: data.error, variant: "destructive" });
            } else {
                setRoles(data || []);
            }
        } catch (error) {
            console.error('Error fetching roles:', error);
            toast({ title: "Error fetching roles", description: "An unexpected error occurred.", variant: "destructive" });
        }
    }, [toast]);
